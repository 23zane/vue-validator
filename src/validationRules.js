...
export function _getRule(rule, formData) {
			...
			func: function (value) {
				...
				if (!ref) {
					if (typeof formData != 'undefined') {
						let data = isRef(formData) ? Object.assign({}, formData.value) : Object.assign({}, formData);
	...
	if (rule.localeCompare('App\\Rules\\CompanyTaxCode') === 0) {
		return {
			key: 'regex',
			func: function (value) {
				const reg = new RegExp(/^([a-zA-Z]{6})([0-9]{2})([a-zA-Z])([0-9]{2})([a-zA-Z])([0-9]{3})([a-zA-Z])$/);
...
export default function useValidationRules(inputs, formData) {
	...
	Object.keys(inputValues).forEach((key) => {
		...
		input.rules.forEach((rule) => {
			const ruleObject = _getRule(rule, formData);
			if (ruleObject) {
				if (typeof rules[key] === 'undefined') {
					rules[key] = {};
				}
